...
CREATE OR REPLACE PROCEDURE WITHDRAW_PROCE(ACCOUNTID IN NUMBER, WITHDRAW IN NUMBER)
IS
...
CREATE OR REPLACE PROCEDURE SP_WITHDRAW
    ...
    WITHDRAW IN NUMBER,
    CURRENT_BALANCE OUT NUMBER
    )
IS
AA_EXISTS INTEGER;
BEGIN 
    SELECT COUNT (A_ACCOUNTID) INTO AA_EXISTS 
    FROM ACCOUNTS A
...
BEGIN 
    ...
    INTO AA_EXISTS
    FROM ACCOUNTS A
    WHERE A.ACCOUNT_ID = A_ACCOUNTID;
    ...
    IF AA_EXISTS > 0 THEN
        UPDATE
            ACCOUNTS
        SET
            A_BALANCE = ACCOUNT_BALANCE + A_AMOUNT
        WHERE 
            ACCOUNT_ID = A_ACCOUNTID;
        --SET RETURN BALANCE
            CUR_BAL := AA_BALANCE;
